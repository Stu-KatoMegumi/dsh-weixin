...
- 流式回复按模型分段：三个连续换行（\n\n\n）= 新气泡，单/双换行不切分；长度上限按当前气泡累计，空闲超时自动兜底
...
- **三个连续换行（\n\n\n）= 一个气泡结束**：模型在输出中用三个连续换行（空两行）主动切气泡，程序把三个换行前的内容立即作为一条消息发送（换行本身不进入微信文案）。单个/双换行只是段落排版，不会触发切分，避免多余断句。此契约写在 `src/prompt/system-prompt.md`，可在设置页修改。
- **长度兜底（按当前气泡累计）**：当前这一个气泡累计超过 `streamFlushChars`（默认 2000 字符）时，在最近的换行/标点处强制切分；每切出一个气泡后从下一个气泡重新计数，不从整条回复头部累计。
- **空闲兜底**：模型超过 `streamFlushMs`（默认 8000 ms）没有新内容时，强制发出当前气泡，避免“没反应”。
