...
- 用户→DSH 会话映射、按会话分隔的对话历史、错误日志和单实例锁持久化
...
- `/help`、`/?`：查看命令（两者功能相同）
- `/new`：创建并切换到新 DSH 会话，同时创建新的 JSONL 历史记录
- `/stop`：取消当前任务
- `/status`：查看连接和会话状态
- `/renew`：立即获取续签二维码图片
...
会话按用户独立维护：同一自然日内继续复用当前 DSH 会话；跨过本机日历日期后，下一条消息会自动创建新 DSH 会话和新的 `history/*.jsonl` 文件。执行 `/new` 会立即切换到新会话，即使仍在同一天也会新建历史文件。旧文件只作为本地归档，不会被重新拼接到新请求中。
...
首次启动会把默认文件复制到频道数据目录（插件模式 `$DSH_HOME/channels/dsh-weixin/prompt/`，独立模式 `session/prompt/`），之后在 DSH“设置 → 微信”页面的 **Prompt 定制** 区编辑这些副本（保存/重置默认）。每个 DSH 会话只在第一条消息前注入一次渲染后的 prompt；如果 prompt 内容发生变化，下一条消息会自动创建新会话并使用新版本，支持 `{date}` 占位符（当前日期）。这样可以避免同一份系统设定在 DSH 会话历史中逐轮重复，造成输入 token 线性膨胀。
